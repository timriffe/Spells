...
\Large
10 trajectories simulated from Dudel & Myrskylä (2017)
...
Clean <- RTraj_clean %>% 
  ...
  gather(key = "clock", value = "mean(duration)", c("y_step_up_inactive","y_step_down_inactive","y_dur_inactive")) %>% 
  mutate(clock)
 ...
 ggplot(mapping = aes(x=x,y=value,color=clock)) +
   geom_line() +
   facet_wrap(~alignment, scales ="free_x")
