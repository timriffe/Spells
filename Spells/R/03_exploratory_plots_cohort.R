...
db_to_summarize_and_plot <- db_tidy %>% 
  ...
  ungroup() %>% 
  filter(ceb >= 2 & afb<40 & maget>=39, !is.na(mage))

db<-db_to_summarize_and_plot
cond<-db$ceb>=2 & db$afb<40 & db$mage == db$maget
table(db$mage[cond])
...
db_to_summarize_and_plot %>% 
  ...
  ggplot(mapping = aes(x = left_par1, 
                       ...
                       color = as.factor(sexf))) + 
  geom_line() + 
  annotate("rect", xmin = 0, xmax = 1, ymin = 0, ymax = 8, alpha = .2) +
  facet_wrap(~afb5) + 
  xlim(0,15) + ylim(0,14) +
  scale_color_manual(labels = c("Boy", "Girl"), values = c("blue", "red")) +
  guides(color=guide_legend(title="First child")) + 
...
dev.print(device=pdf, 'mt_second_gir_by_sex_first_cohort.pdf', width=7, height=5)
