...
# currentpath <- c("C:\\Users\\Erik\\Dropbox\\MAFE")


# save par defaults for resetting -----------------------------------------

def.par <- par(no.readonly = TRUE)

# Libraries ---------------------------------------------------------------
pkglist <- c('car', 'foreign', 'TraMineR', 'sfsmisc', 'xtable', 'estout', 'cluster', 'Hmisc', 'vcd', 'plyr','mlogit','nnet','effects','stargazer', 'RColorBrewer','AER',"here" )
...
for (i in pkglist) {
  if(!is.element(i, installed.packages()[,1])){
    ...
    print(paste(i,"library already installed"))
  }
...
seqdplot(legstat.17nov.seq,withlegend=TRUE,with.missing=TRUE,cex.legend=.8,xtlab=c(1:54))
