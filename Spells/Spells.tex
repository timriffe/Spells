...
The following exercise is done for females only. We arrange transition probabilities into standard Markov transition matrices, and then simulate 50,000 trajectories using the \texttt{rmarkovchain} package \citep{spedicato2017} and assuming that everyone starts healthy at age 15. Each trajectory consists in 65 time steps.

\subsubsection{Grammatical operations}
 
We illustrate the disability clocks calculated using 10 randomly generated trajectories starting at age 50 and ignoring younger ages. In order to calculate the mean duration of spells starting in age $x$, we impute the spell duration in the first time step of each disability spell, following the pattern in Fig.~\ref{fig:disdurex}. This is essentially a single step because we retain the default age alignment.
\begin{figure}
    \centering
    \includegraphics[scale=.7]{Spells/Figures/DisTrajExample.pdf}
    \caption{Ten example trajectories, with disability spell duration imputed in the first time step of each spell. Light green is healthy and brown is disabled. Ages below 50 are ignored in this illustration.}
    \label{fig:disdurex}
\end{figure}
In this example, only individuals deceased before age 80 are shown, but in the full sample some individuals reach age 80 in a state of disability, which may continue into higher ages if we had the transition probabilities to simulate so far. This introduces a downward bias in spell duration that grows in magnitude with the approach to age 80. For this reason, we right truncate results for aggregates at age 70. To aggregate macro patterns from this, we take conditional means within each age, throwing out any values that are not the first time step in a disability spell. This produces two age patterns, one for the highest and one for the lowest income quintile. 
