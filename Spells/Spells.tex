...
Two caveats to the use of alignments and clocks are worth noting. First, aggregate measures based on clocks and alignment operations can be biased due to the left- or right-censoring of trajectories, i.e., when the observation window of sequences does not cover the full period over which events (change in states) can be observed. In the previous section, we discussed an example that may be affected by left censoring because we do not know the employment status of individuals before age 50. To partially correct this, the data were simulated so that all individuals are employed by age 50, making all mean calculations based on stepping clocks, conditional on being employed at this age. Likewise, since not all individuals die before age 80, descending clocks (e.g., mean time to death) are affected by right censoring, especially at ages that are close to the end of the observation window (age 80). Hence, users should be careful about the potential bias of the aggregate measures and make clear the conditional nature of them when necessary.

Second, users should be careful when conducting alignment operations as some observations will not be included in the aggregate measures if they do not experience the event that serves to align the trajectories. For example, individuals without spells of unemployment will not be included in the calculation of the mean time to next employment after a first job loss. Although these examples seem trivial, we encourage careful thinking when defining alignments, clocks, and aggregate measures.

The following section presents two examples where clocks and alignment operations are used to examine sequence data. In the first example we rely on X clock measures and X alignment(s) to examine XYZ. In the second example we use three clocks and one alignment operation to study the mean duration of birth intervals according to the age of the mother at first birth, and the sex of the first child.
...
Population level studies of disability typically report values such as mean time spent disabled, either derived from a multistate model of disability \citep{crimmins2009change} or derived using the Sullivan method \citep{Sullivan1970,Crimmins1997}. It is rare to see other model statistics reported, even for results based on simulated disability trajectories. Especially for the case of describing group inequalities, it may be interesting to measure quantities beyond health expectancies, such as the age pattern of disability spell length. Do new bouts of disability tend to get longer or shorter with age? Who has longer spells of disability, the rich or the poor?

We aim to show some preliminary results using and example of an age-structured clock (the default alignment).
Long term disability trajectories over the life course of individuals are typically not recorded, except in rare cohort studies, or in long-running health registers. To produce our base set of trajectories, we first estimate transition probabilities, then we simulate over a wide age range. We then demonstrate clock aggregation for the case of disability spell duration. This is proposed as an interesting metric for health inequalities.
...
For all disability measures we produce, we simulate  right censoring can bias results if due care is not taken.
