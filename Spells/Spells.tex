...
such as state occupancies or the moments thereof. Recently formulas
have been derived to compute the average number of spells for a given state,
which when combined with the average state occupancy yields the average spell
...
other patterns of different state episodes. 
\end{abstract}
...
(incluiding the aforementioned) so broad and flexible as to fill the lifetime of
an imaginary matrix-lover.
I will procede using simulations rather than matrix calculations because it will
...
\emph{realignments}, resulting in a large set of age-like patterns of state
episodes. Simulation is not strictly required if an empirical population of
individual sequences is on hand, but these would pertain to a non-stationary
population.

\section{Data}
The point of departure for all calculations is a
...
be done with any age-stage matrix. One can indeed also do the same with
...
Inlcude a big set of patterns from the example matrix. Somehow. Systematically.
Like a 5x5 panel plot, where five clocks in columns and five alignments in rows.
Some combinatorics to see the researcher degrees of freedom too.

\section{Discussion}
As stated the purpose of episode clocks and sequencec realignment is to detect
important patterns in data that are likely to otherwise go unnoticed. Some
reasonable priors might include that i) life course events condition each other;
ii) temporal proximity to life course transitions is likely to be an important
